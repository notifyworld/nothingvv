...
router.post("/create", async (req, res) => {
  ...
  const { framework = "nextjs" } = req.body; // Allow framework selection

  try {
    console.log(`Creating container with framework: ${framework}`);
    const imageName = await dockerService.buildImage(containerId, framework);
    ...
    await dockerService.cleanupImage(containerId, framework);
...
router.get("/:containerId/export", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="soul-project-${containerId.slice(0, 8)}.zip"`
    );
